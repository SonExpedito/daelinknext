...
cred_path = os.path.join(os.path.dirname(__file__), '../credencial/credencial.json')
...
def recommend():
    try:
        ...
        job_description = data.get('descricao', '')

        if not job_title and not job_description:
            return jsonify({"error": "Campos 'trabalho' ou 'descricao' são necessários."}), 400

        job_index, _ = find_job_index_by_similar_description_and_title(
            job_title, job_description, jobs, threshold=0.2
        )

        if job_index is None:
            return jsonify([]), 200   # retorna lista vazia se não achou
        ...
        return jsonify({"error": "Erro ao recomendar"}), 500
