  ...
  useEffect(() => {
    const getVagas = async () => {
      try {
        ...
        // Filtra apenas as vagas abertas
        const vagasAbertas = response.data.filter(v => v.status === "Aberta");
          ...
          {filteredVagas.length > 0 ? (
            filteredVagas.map((vaga) => (
              <button
                ...
                type="button"
                className="w-90 h-40 p-4 rounded-3xl background-primary flex items-center justify-center overflow-hidden hover-size cursor-pointer card-border focus:outline-none"
              >
                <div className="h-full w-[48%] flex items-center justify-center ">
                  <img
                    src={vaga.img ||vaga.empresa?.imageProfile || "/errors/bannererror.png"}
                    className="object-cover h-[85%] w-64 rounded-3xl"
